...
class SellerActiveItemsWebTests extends grails.util.WebTest{
    ...
    void assertHasItemListed(sellerItems){
    	sellerItems.each { 
    		...
    		verifyXPath xpath:"//td[@id='currentPrice$it.id']", text:it.currentPrice
    		verifyXPath xpath:"//td[@id='quantitySelled$it.id']", text:it.quantitySelled
    		verifyXPath xpath:"//td[@id='offersReceived$it.id']", text: it.offersReceived
    		verifyXPath xpath:"//td[@id='quantityAvailable$it.id']", text:it.quantityAvailable
    	}
    }
}